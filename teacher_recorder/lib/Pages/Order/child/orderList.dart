...
class _orderPageState extends State<orderPage> {
  ...
  void getNewData() async{
    ...
    DataUtils.MatchingListData({"page": pageIndex, "pageSize": 10, },
        success: (result) {
          print(result);
  ...
  Widget _cellItem(){
    ...
    if(groupData.length==0){
      return Container(
        ...
        //    alignment: Alignment.center,
        child:   Text("附近没有合适的学生...",textAlign: TextAlign.center,style:  TextStyle(fontSize: 17.0,color: Colors.grey)),
      );
  ...
  Widget _ContentItem(Record model){
          ...
          children: <Widget>[
            ...
            Row(
              ...
              children: <Widget>[
                ...
                new FlatButton(
                  color:model.applyState == "1"? KColor.kTextGrayColor:KColor.NavMainColor,
                  textColor: Colors.white,
                  minWidth: 50,
                  height: 32,

                  child: new Text(model.applyState == "1"?'已接单':"接单"),
                  onPressed: () {
                    if(model.applyState == "1"){
                      JhProgressHUD.showText("已接单。。");
                    }else{
                      DataUtils.GrabOrderData({"matchingId" : model.id},success: (data){
                        print("返回数据： ============>>>>>>  $data");
   ...
   void robData() async{
     ...
     DataUtils.GrabOrderData({"matchingId" : ""},success: (data){
